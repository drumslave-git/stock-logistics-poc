...
3. Delivery info is computed from the allocation: total cargo weight vs. helicopter `maxPayloadKg` → number of vehicles; distance vs. `maxSpeed` → duration. Distance is **straight-line (haversine)** — helicopters don't follow roads. For multi-source orders, legs fly **in parallel**, so order duration = the longest leg.
